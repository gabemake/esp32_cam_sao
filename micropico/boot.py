...
while not slug_bus:
    try:
        slug_bus =  which_bus_has_device_id(SLUG_ADDRESS)[0]
    except:
        pass
    time.sleep_ms(100)
    slugbus_counter = slugbus_counter + 1
    if slugbus_counter > 50:
        break
if not slug_bus:
...
esp_ai_bus = None
espbus_counter = 0
while not esp_ai_bus:
    try:
        esp_ai_bus =  which_bus_has_device_id(ESP_AI_ADDRESS)[0]
    except:
        pass
    time.sleep_ms(100)
    espbus_counter = espbus_counter + 1
    if espbus_counter > 50:
        break
if not esp_ai_bus:
    print(f"Warning: esp ai not found.")
else:
    print(f"esp ai found")

if esp_ai_bus and slug_bus:
    slug_mode(slug_bus, 2)


def esp_ai_read(bus):
    return(esp_ai_bus.readfrom_mem(ESP_AI_ADDRESS, 0, 1)[0])


def slug_rgb(bus, r, g, b):
    """RGB color on the central display.  Each 0-255"""
    slug_bus.writeto_mem(SLUG_ADDRESS, 1, bytes([r]))
    slug_bus.writeto_mem(SLUG_ADDRESS, 2, bytes([g]))
    slug_bus.writeto_mem(SLUG_ADDRESS, 3, bytes([b]))

def slug_mode(bus, mode):
    slug_bus.writeto_mem(SLUG_ADDRESS, 0, bytes([mode]))
